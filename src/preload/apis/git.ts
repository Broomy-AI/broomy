...
export type GitApi = {
  ...
  showBase64: (repoPath: string, filePath: string, ref?: string) => Promise<string>
  ...
  fetchReviewPrHead: (repoPath: string, prNumber: number, targetBranch?: string) => Promise<{ success: boolean; error?: string }>
  syncReviewBranch: (repoPath: string, branchName: string, prNumber: number) => Promise<{ success: boolean; error?: string }>
  isMergedInto: (repoPath: string, ref: string) => Promise<boolean>
  hasBranchCommits: (repoPath: string, ref: string) => Promise<boolean>
  pullOriginMain: (repoPath: string) => Promise<{ success: boolean; error?: string }>
  isBehindMain: (repoPath: string) => Promise<{ success: true; behind: number; defaultBranch: string } | { success: false; error: string }>
...
export const gitApi: GitApi = {
  ...
  showBase64: (repoPath, filePath, ref) => ipcRenderer.invoke('git:showBase64', repoPath, filePath, ref),
  ...
  fetchReviewPrHead: (repoPath, prNumber, targetBranch) => ipcRenderer.invoke('git:fetchReviewPrHead', repoPath, prNumber, targetBranch),
  syncReviewBranch: (repoPath, branchName, prNumber) => ipcRenderer.invoke('git:syncReviewBranch', repoPath, branchName, prNumber),

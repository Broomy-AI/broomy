...
export function ReviewStatusChip({ status }: { status: 'pending' | 'reviewed' }) {
  if (status === 'reviewed') {
    return (
      <span className="text-3xs px-1.5 py-0.5 rounded font-semibold bg-green-500/20 text-green-400 flex-shrink-0">
        Reviewed
      </span>
    )
  }
  return (
    <span className="text-3xs px-1.5 py-0.5 rounded font-semibold bg-cyan-500/20 text-cyan-400 flex-shrink-0">
      Review
    </span>
  )
}

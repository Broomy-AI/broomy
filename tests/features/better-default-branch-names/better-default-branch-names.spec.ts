 ...
 * GitHub issue. Common English words are stripped for a clean slug, and the
 * branch is namespaced under `issue/<number>-` so issue worktrees group in the
 * repo's `issue/` folder and stay unique — e.g.
 * "issue/42-add-mode-toggle-user-settings-panel".
 *
  ...
  await generateFeaturePage(
    {
      ...
      description:
        'When creating a branch from a GitHub issue, the branch name is generated by ' +
        'stripping common English words (the, of, for, in, etc.) from the issue title for a ' +
        'clean slug, then namespacing it under `issue/<number>-` so issue worktrees land in the ' +
        'repo\'s issue/ folder and stay unique — e.g. ' +
...
test.describe.serial('Feature: Better Default Branch Names', () => {
  ...
  test('Step 2: Select an issue — see the cleaned-up branch name', async () => {
    ...
    // Exact generated slug for issue #42: filler words stripped, namespaced under issue/<number>-.
    expect(branchValue).toBe('issue/42-add-mode-toggle-user-settings-panel')
    ...
    steps.push({
      ...
      caption: 'Clean slug, namespaced under issue/<number>-',
      description:
        'The full issue title is "Add support for the dark mode toggle in the user settings panel". ' +
        `The generated branch name is "${branchValue}" — filler words like "support", "for", ` +
        '"the", "dark", and "in" are stripped, and the concise slug is namespaced under "issue/42-" ' +
        'so it groups in the repo\'s issue/ folder and stays unique.',
    })
  })
  ...
  test('Step 3: Select a different issue to see another example', async () => {
    ...
    // Exact generated slug for issue #17.
    expect(branchValue).toBe('issue/17-fix-crash-happens-clicking-notification')
    ...
    steps.push({
      ...
      caption: 'Another verbose title reduced to its key terms, under issue/',
      description:
        'The issue title "Fix the crash that happens when clicking on an empty notification list" ' +
        `becomes "${branchValue}" — filler words like "the", "that", "when", "on", "an", "empty", ` +
        'and "list" are stripped, and the branch is namespaced under "issue/17-".',
    })
  })
})
